    ...
    with SessionLocal() as db:
        try:
            ...
            metrics_data = {
                ...
                'metric_contributors': float(odoo_metrics.metric_contributors or 0),  # 修复：使用正确的列名
                'metric_issues_new': float(odoo_metrics.metric_issues_new or 0),
                'metric_change_requests_new': float(odoo_metrics.metric_change_requests or 0),  # 修复：使用正确的列名
                'metric_bus_factor': float(odoo_metrics.metric_bus_factor or 0),
                # 注意：metric_hhi 列不存在，我们跳过它
                'raw_payloads': {}
            }
